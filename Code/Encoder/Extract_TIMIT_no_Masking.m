...
train_dir='F:\TIMIT no masking\timit\train';
test_dir='F:\TIMIT no masking\timit\test';
...
for i=3:length(train_sub1)
    ...
    for j=3:length(train_sub2)
        ...
        for k=3:4:length(train_sub3) %interval 6, original 4
                ...
                [spike_output,binary_output]=encode_single_wav_NoMask(wav_file); 
    ...
    for j=3:length(test_sub2)
        ...
        for k=3:4:length(test_sub3)
            ...
            try
                ...
                %wrd_file=train_sub3(k+3).name;
                %test_TIMIT_data{counter,1}=wav_file;
